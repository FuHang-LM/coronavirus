...
def insertData(cons, txt, url):
    ...
    try:
        cue.execute(
            "INSERT INTO hubei(txt, url) value (%s,%s)",
            [txt, url]
        )
...
def read():
    # filename = "../../reptile/txt_province/hubei.txt"
    filename = "../reptile/txt_province/hubei.txt"  # 脚本运行路径
...
def format_data():
    cursor = con.cursor()
    cursor.execute("truncate table hubei")
    # print('清楚表数据成功')
    con.commit()
    cursor.close()
    # con.close()
